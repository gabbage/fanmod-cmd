...
esuThread::esuThread(const wxString & inputfile_p, bool directed_p, bool has_vertex_colors_p,
                     ...
                     wxFrame * frame_p)
                     : wxThread(),
                       vertex_color_matters(vertex_color_matters_p),
                       edge_color_matters(edge_color_matters_p)
{
    inputfile = inputfile_p.mb_str();
    directed = directed_p;
    has_edge_colors = has_edge_colors_p;
    has_vertex_colors = has_vertex_colors_p;
    G_N = G_N_p;
    fullenumeration = fullenumeration_p; 
    for (int i=0; i<8; ++i){ // create a deep copy of the prob array
        prob[i] = prob_p[i];
    }
    TREESMPLS = treesmpls_p;
    num_r_nets = num_r_nets_p;
    random_type = random_type_p;
    num_exchanges = int(num_exchanges_p);
    num_tries = int(num_tries_p);
    reest_size = reest_size_p;
    outputfile = outputfile_p.mb_str();
    text_output = text_output_p;
    gen_dumpfile = gen_dumpfile_p;
    frame = frame_p;
}
              
void *esuThread::Entry()
{
    ...
    // Event to be send when a net was finished
    wxCommandEvent net_event(wxEVT_COMMAND_MENU_SELECTED, ID_NET_FINISHED);
    net_event.SetInt(netevent_number);
    ...
    // Write the things we have just written to the outfile to the info-stringstream as well
    info << "Network name: " << inputfile.c_str() << "\n";
    info << "Network type: " << (maing.directed? "Directed" : "Undirected") << "\n";
    info << "Number of nodes: " << maing.n << "\n"
            << "Number of edges: " << maing.m << "\n";
    info << "Number of single edges: " << maing.num_dir_edges << "\n"
            << "Number of mutual edges: " << maing.num_undir_edges << "\n";
    if (has_vertex_colors) info << "Number of vertex colors: " << maing.num_vertex_colors << "\n";    
    if (has_edge_colors) info << "Number of edge colors: " << maing.num_edge_colors << "\n";
    info << "\nAlgorithm: " << (fullenumeration ? "enumeration" : "sampling") << "\n";
    if (!fullenumeration) {
    ...
    info << "Subgraph size: " << G_N << "\n\n";
    if (num_r_nets == 0)
    {
        info << "No random graphs are generated\n";
    } else {
        info << "Will generate " << num_r_nets << " random networks\n"
                << "   with " << type_str.c_str() << "\n"
                << "   " << num_exchanges << " exchanges per edge and "
                <<  num_tries << " tries per edge\n\n";
    }

    // If the user cancelled, send "NO_SUCCESS" and exit
    if (TestDestroy()){
        wxCommandEvent event(wxEVT_COMMAND_MENU_SELECTED,
                         ID_THREAD_NO_SUCCESS);
        wxPostEvent(frame, event);
	...
	if (TREESMPLS > 0) {
        ...
        info << "Approximate number of subgraphs: " << numtrees
             << " (based on " << TREESMPLS << " samples)\n"; 
	    if (!fullenumeration){
            SMPLS = calc_expected_samples(double(numtrees), G_N, prob);
            info << "Expected number of sampled subgraphs: " << SMPLS << endl;
        }
    }
	
	// Throw an event with the graph information
	wxCommandEvent info_event(wxEVT_COMMAND_MENU_SELECTED, ID_THREAD_INFO);
	info_event.SetString(wxString(info.str().c_str(),wxConvUTF8));
    wxPostEvent(frame, info_event);
    ...
    vector<subgraph> subgraphdump;
       ...
       sampling_time += sampling(maing, v_extension, G_N, fullenumeration, prob, 
                                 ...
                                 count_subgr[nets_ctr], frame, this, rand,
       ...
       if (nets_ctr == 0){
           info << count_subgr[0] << " subgraphs were "
       ...
       // Notify the main thread, that a network was sampled
       if (nets_ctr % netevent_number == 0)
           wxPostEvent(frame, net_event);
    ...
    // Send the "finished" event to the Main-Thread
    wxCommandEvent event(wxEVT_COMMAND_MENU_SELECTED,
                         ID_THREAD_FINISHED);
    wxPostEvent(frame, event);
    return NULL;
